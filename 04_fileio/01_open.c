...
O_CREAT        v3        Создание файла, если он еще не существует, если указывается O_CREAT то нужно указавать третий аргумент права файла:
...
O_LARGEFILE              Используется в 32-разрядных системах для открытия больших файлов(превышает 2Гбайт). (либо использовать вызов open64())
...
O_APPEND       v3        Записи добавляются исключительно в конец файла (гарантирует атомарность)
...
O_NONBLOCK     v3        Открытие в неблокируемом режиме (возвращает ошибку EAGAIN или EWOULDBLOCK)
