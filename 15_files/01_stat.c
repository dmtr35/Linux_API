...
void printf_stat(struct stat *statbuf)
{
    printf(
        ...
        "%-10s  %-10s: %ld\n"
        "%-10s  %-10s: %ld\n",
        "dev_t", "st_dev", statbuf->st_dev,          // Идентификатор устройства, на котором находится файл      st_dev    : 64769                             
        "ino_t", "st_ino", statbuf->st_ino,          // Номер индексного дескриптора файла                       st_ino    : 5776159             
        "mode_t", "st_mode", statbuf->st_mode,        // Тип файла и права доступа                                st_mode   : 33204 
        "nlink_t", "st_nlink", statbuf->st_nlink,      // Количество (жестких) ссылок на файл                      st_nlink  : 1             
        "uid_t", "st_uid", statbuf->st_uid,          // Пользовательский ID владельца файла                      st_uid    : 1000             
        "gid_t", "st_gid", statbuf->st_gid,          // Групповой ID владельца файла                             st_gid    : 1000     
        "dev_t", "st_rdev", statbuf->st_rdev,        // Идентификаторы файлов устройств                          st_rdev   : 0         
        "off_t", "st_size", statbuf->st_size,        // Общий размер файла (в байтах)                            st_size   : 32     
        "blksize_t", "st_blksize", statbuf->st_blksize,  // Оптимальный размер блока для ввода-вывода (в байтах)     st_blksize: 4096                             
        "blkcnt_t", "st_blocks", statbuf->st_blocks,    // Количество отведенных блоков (по 512 байт)               st_blocks : 8                     
        "time_t", "st_atime", statbuf->st_atime,      // Время последнего доступа к файлу                         st_atime  : 1782223799         
        "time_t", "st_mtime", statbuf->st_mtime,      // Время последнего изменения файла                         st_mtime  : 1781455645         
